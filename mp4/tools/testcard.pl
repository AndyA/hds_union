...
use constant FPS  => 25;
use constant FONT => '/Users/andy/Library/Fonts/Envy Code R.ttf';
...
sub testcard {
  ...
  for my $fr ( 0 .. $a{frames} - 1 ) {
...

    {
      ...
      my $bw    = $a{width} / 2;
      my $pos   = max( 0, min( $bw, $bw * ( $fr - $a{leadin} ) / $scale ) );

      for my $dy ( 30, $a{height} - 1 - 30 ) {
        $img->filledRectangle( 0,                $dy - 10, $pos,      $dy + 10, $fg );
        $img->filledRectangle( $a{width} - $pos, $dy - 10, $a{width}, $dy + 10, $fg );
      }
    }

    file( $out, sprintf $a{template}, $fr )->openw->print( $img->jpeg( 90 ) );
  }
  print "\n";
}
